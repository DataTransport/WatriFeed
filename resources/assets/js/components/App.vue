                ...
                <div class="card mt-4">
                    <v-map ref="map" id='map' :zoom="zoom" :maxZoom="tileProvider.maxZoom" :center="center">
                        ...
                        <v-marker-cluster ref="markerCluster" :options="clusterOptions">
                            <v-marker v-for="l in  $root.fuzzySearch" :key="l.id" :lat-lng="l.latlng" ref="item"
                                      :icon="$root.iconMarker(l.image)">
...
//                tile provider
        ...
        methods: {
            getLocation(location) {
                var vm = this;
                var index = _.findIndex(vm.$root.$data.locations, ['id', location.id]);
