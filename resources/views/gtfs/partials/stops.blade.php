                ...
                <div style="border-bottom: 1px solid #fff;background: #000000;color: white;" class="panel-heading">
                    ...
                    <div class="panel-options">
                        <span class="badge badge-success" style="color: #000000; font-weight: bold; font-size: 10px">
                            Records {{ $stops->firstItem() }} - {{ $stops->lastItem() }} of {{ $stops->total() }} (for page {{ $stops->currentPage() }} )
                        </span>
                ...
                <div class="panel-body">
                    ...
                    <table class="table table-bordered datatable" id="stops_data" data-order='[[ 1, "asc" ]]'
                        ...
                        @foreach ($stops as $stop)
                            <tr class="odd gradeX" id="{{$stop->stop_id}}" onclick='markerFunction($(this)[0].id); test()'>
                                <td class="update {{$stop->id}}" data-id="{{$stop->id}}"
                                    data-column="stop_id">{{$stop->stop_id}}</td>
                                <td class="update {{$stop->id}}" data-id="{{$stop->id}}"
                                    data-column="stop_code">{{$stop->stop_code}}</td>
                                <td class="update {{$stop->id}}" data-id="{{$stop->id}}"
                                    data-column="stop_name">{{$stop->stop_name}}</td>
                                <td class="update {{$stop->id}}" data-id="{{$stop->id}}"
                                    data-column="stop_desc">{{$stop->stop_desc}}</td>
                                <td class="update {{$stop->id}}" data-id="{{$stop->id}}"
                                    data-column="stop_lat">{{$stop->stop_lat}}</td>
                                <td class="update {{$stop->id}}" data-id="{{$stop->id}}"
                                    data-column="stop_lon">{{$stop->stop_lon}}</td>
                                <td class="update {{$stop->id}}" data-id="{{$stop->id}}"
                                    data-column="zone_id">{{$stop->zone_id}}</td>
                                <td class="update {{$stop->id}}" data-id="{{$stop->id}}"
                                    data-column="stop_url">{{$stop->stop_url}}</td>
                                <td class="update {{$stop->id}}" data-id="{{$stop->id}}"
                                    data-column="location_type">{{$stop->location_type}}</td>
                                <td class="update {{$stop->id}}" data-id="{{$stop->id}}"
                                    data-column="parent_station">{{$stop->parent_station}}</td>
                                <td class="update {{$stop->id}}" data-id="{{$stop->id}}"
                                    data-column="stop_timezone">{{$stop->stop_timezone}}</td>
                                <td class="update {{$stop->id}}" data-id="{{$stop->id}}"
                                    data-column="wheelchair_boarding">{{$stop->wheelchair_boarding}}</td>
                                <td class="update {{$stop->id}}" data-id="{{$stop->id}}"
                                    data-column="level_id">{{$stop->level_id}}</td>
                                <td class="update {{$stop->id}}" data-id="{{$stop->id}}"
                                    data-column="platform_code">{{$stop->platform_code}}</td>

                                <td>
                                    <button style="display: none;" type="button" name="save_btn"
                                            class="btn btn-success btn-sm save_btn{{$stop->id}} save_btn"
                                            data-rowid="{{$stop->id}}">save
                                    </button>
                                    <button type="button" name="edit_btn"
                                            class="btn btn-info btn-sm edit_btn{{$stop->id}} edit_btn_"
                                            data-rowid="{{$stop->id}}"
                                            data-stop_id="{{$stop->stop_id}}"
                                            data-stop_code="{{$stop->stop_code}}"
                                            data-stop_name="{{$stop->stop_name}}"
                                            data-stop_desc="{{$stop->stop_desc}}"
                                            data-stop_lat="{{$stop->stop_lat}}"
                                            data-stop_lon="{{$stop->stop_lon}}"
                                            data-zone_id="{{$stop->zone_id}}"
                                            data-stop_url="{{$stop->stop_url}}"
                                            data-location_type="{{$stop->location_type}}"
                                            data-parent_station="{{$stop->parent_station}}"
                                            data-stop_timezone="{{$stop->stop_timezone}}"
                                            data-wheelchair_boarding="{{$stop->wheelchair_boarding}}"
                                            data-level_id="{{$stop->level_id}}"
                                            data-platform_code="{{$stop->platform_code}}"
                                    >
                                        <i class="fa fa-edit"></i>
                                    </button>
                                    <button type="button" name="delete" class="btn btn-danger btn-sm delete"
                                            id="{{$stop->id}}"><i class="fa fa-trash"></i></button>
